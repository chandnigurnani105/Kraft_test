...
class MenuController extends BaseController
{
    ...
    {
        $rootEvents = MenuItem::whereNull('parent_id')->with('children')->get();

        $menuItems = [];
        foreach ($rootEvents as $event) {
            $menuItem = $event->toArray();
            $menuItem['children'] = $this->buildChildren($event);
            $menuItems[] = $menuItem;
        }

        return response()->json($menuItems);
        return view('menus/index',['menus'=>$menuItems]);

    }
    private function buildChildren($event)
    {
        $children = $event->children->toArray();
        foreach ($children as &$child) {
            $child['children'] = $this->buildChildren(MenuItem::find($child['id']));
        }
        return $children;
    }
